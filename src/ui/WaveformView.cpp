...
void WaveformView::resized()
{
    ...
    // --- Trim marker clamping guard on resize ---
    auto sampleSnap = processor.sampleData.getSnapshot();
    if (sampleSnap) {
...
// ---- KEY LOGIC ORDER: TRIM -> EDGE DRAG -> MPC ADD -> SELECT SLICE ----
void WaveformView::mouseDown (const juce::MouseEvent& e)
{
    ...
    // (1) ---- TRIM MODE: drag handles/markers ----
    if (trimMode)
    {
        const int x1 = sampleToPixel (trimInPoint);
        const int x2 = sampleToPixel (trimOutPoint);
    ...
    // (2) ---- EDGE DRAG (normal mode only, not Add Slice, not Alt) ----
    const auto& ui = processor.getUiSliceSnapshot();
    int sel = ui.selectedSlice;
    int num = ui.numSlices;
    ...
    // (3) ---- MPC-STYLE ADD MARKER: Add Slice mode or Alt ----
    ...
    // (4) ---- SELECT SLICE ON BACKGROUND CLICK ----
    for (int i = 0; i < num; ++i)
    {
...
// mouseDrag/mouseUp and other interaction functions remain UNCHANGED from your previous code
...
void WaveformView::enterTrimMode (int start, int end)
{
    ...
    // --- Clamp after enter (for robust resize handling) ---
    auto sampleSnap = processor.sampleData.getSnapshot();
    if (sampleSnap) {
