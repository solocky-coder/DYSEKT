    ...
    if (e.mods.isRightButtonDown())
    {
        ...
        menu.showMenuAsync (
            ...
            [this, targetSlice, hasSliceItems] (int result)
            {
                ...
                {
                    // ── Rename Slice ──────────────────────────────────────────
                    // Delegate to the editor which hosts a themed in-plugin overlay
                    // (replaces AlertWindow which appeared outside the plugin window)
                    ...
                    return;   // overlay is async — skip repaint() below
                }
                repaint();
            });
        return;
